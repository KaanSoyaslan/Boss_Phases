...
public class Boss : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isShooting == false) //ateþ etme waiter
        {
    ...
    {
        if (other.CompareTag("Pbullet")) //player bullet
        {
            enemyhealth--;
            if (enemyhealth <= iki)
            {
    ...
    IEnumerator Shoot() //evreye göre ateþ etme
    {
        isShooting = true;
        if (gunnum == 1)
        {
    ...
    private void Bomb() //etrafa yayýlma
    {
    ...
    void FireEnemyBullet() //güdümlü mermi
    {
